-- Personality Quark: Lexicon table
-- Vocabulary fingerprint — words/phrases to favor, avoid, or use situationally.

CREATE TABLE IF NOT EXISTS personality_quark_lexicon (
...
CREATE INDEX IF NOT EXISTS idx_pq_lexicon_profile
    ON personality_quark_lexicon(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pq_lexicon_type
    ON personality_quark_lexicon(profile_slug, usage_type);
CREATE INDEX IF NOT EXISTS idx_pq_lexicon_freq
    ON personality_quark_lexicon(profile_slug, frequency);
